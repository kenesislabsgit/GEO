...
export function MethodologyPanel({ scan }: { scan: PublicReportDTO["scan"] }) {
  return <section aria-label="Run methodology" className="mx-auto max-w-6xl px-4 py-8 md:px-6">
    <details className="rounded-xl border border-border p-4">
      <summary className="cursor-pointer text-sm font-semibold">How this audit was run</summary>
      <p className="mt-3 text-xs text-muted-foreground">Scoring method {scan.methodologyVersion}</p>
      <dl className="mt-4 grid gap-4 text-xs sm:grid-cols-2">
        <div><dt className="font-medium">AI assistant and model</dt><dd className="mt-1 space-y-1 break-words text-muted-foreground">{scan.sampling.models.length ? scan.sampling.models.map((entry) => <p key={`${entry.provider}:${entry.model}`}>{providerDisplayName(entry.provider)} · {entry.model}</p>) : "Not recorded"}</dd></div>
