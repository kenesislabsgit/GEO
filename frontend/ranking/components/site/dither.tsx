...
const DEFAULT_WAVE_BG: Rgb = [0, 0, 0];
const DEFAULT_COLOR_NUM = 4;
const DEFAULT_PIXEL_SIZE = 2;
const DEFAULT_MOUSE_RADIUS = 1;
...
void main() {
  ...
  vec3 col = mix(waveBg, waveColor, clamp(f, 0.0, 1.0));
...
class RetroEffectImpl extends Effect {
  ...
  get colorNum(): number {
    return (this.uniforms.get("colorNum")?.value as number) ?? 4;
  }
  ...
  get pixelSize(): number {
    return (this.uniforms.get("pixelSize")?.value as number) ?? 2;
  }
}
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    waveBg: new THREE.Uniform(new THREE.Color(...waveBg)),
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevBg.current.every((v, i) => v === waveBg[i])) {
      u.waveBg.value.set(...waveBg);
      prevBg.current = [...waveBg];
    }
...
class WebGlGate extends Component<{ children: ReactNode }, { ok: boolean }> {
  state = { ok: true };
